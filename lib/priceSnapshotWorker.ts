...
const TRACKED_CHAINS: { test: (n: string) => boolean; label: string }[] = [
  { test: (n) => n.includes('carrefour'), label: 'Carrefour' },
  { test: (n) => n.includes('changomas') || n.includes('chango mas'), label: 'Changomas' },
  { test: (n) => n.includes('disco'), label: 'Disco' },
  { test: (n) => n.includes('jumbo'), label: 'Jumbo' },
  { test: (n) => n.includes('coto'), label: 'Coto' },
  { test: (n) => n.includes('dia') || n.includes('día'), label: 'Dia' },
];
...
async function fetchReferenceStores(): Promise<ReferenceStore[]> {
  ...
  const res = await fetch(url, { headers: PRECIOS_CLAROS_HEADERS, cache: 'no-store' });
  ...
  for (const chain of TRACKED_CHAINS) {
    const match = list.find((s) => s?.banderaDescripcion && chain.test(String(s.banderaDescripcion).toLowerCase()));
    const id = match?.id;
...
async function fetchPricesForProduct(
  ...
  for (const suc of data?.sucursales || []) {
    ...
    const precio = (promo.length ? Math.min(...promo) : undefined) ?? precioLista;
...
export async function runPriceSnapshotBatch(): Promise<SnapshotRunResult> {
  ...
  for (const { ean, nombre } of products) {
    try {
      // Precio más bajo de la corrida ANTERIOR para este producto (todas
      // las filas de esa corrida comparten el mismo captured_at, así que
      // alcanza con mirar las últimas hasta TRACKED_CHAINS.length filas).
      ...
      const prevBest = prevRows?.length ? Math.min(...prevRows.map((r) => r.precio)) : null;
