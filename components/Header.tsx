...
export default function Header({
  ...
  // El globito de "creá tu cuenta" solo se cerraba con su × o abriendo el
  // menú. Tocar en cualquier otro lado no hacía nada y quedaba ahí tapando
  // media pantalla. Ahora se va al tocar afuera, al scrollear o con Escape,
  // como cualquier cosa que aparece encima del contenido.
  useEffect(() => {
    if (!showAccountHint || accountOpen) return;
    function dismiss(e: MouseEvent | TouchEvent) {
      if (accountRef.current && accountRef.current.contains(e.target as Node)) return;
      onDismissAccountHint?.();
    }
    function onEsc(e: KeyboardEvent) {
      if (e.key === 'Escape') onDismissAccountHint?.();
    }
    function onScroll() {
      onDismissAccountHint?.();
    }
  ...
  }, [showAccountHint, accountOpen, onDismissAccountHint]);
      ...
      <div className="head-row">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div className="account-menu" ref={accountRef}>
            ...
            {!accountOpen && showAccountHint && (
              <div className="account-hint">
                <button className="account-hint-close" aria-label="Cerrar" onClick={onDismissAccountHint}>×</button>
                Creá tu cuenta o iniciá sesión para guardar tu carrito y verlo desde cualquier celular.
                <div className="account-hint-actions">
                  <button
                    className="primary"
                    onClick={() => {
                      onDismissAccountHint?.();
