...
// atributo data-theme se pintaba en un <div> intermedio. Eso traía tres
// problemas reales:
//
//   1. Se perdía en cada recarga: elegías oscuro, volvías a entrar y estaba
//      claro otra vez.
//   2. Ignoraba la preferencia del sistema operativo, que es lo que espera
//      cualquiera que tenga el celular en modo oscuro.
//   3. Al estar en un div y no en <html>, el fondo del documento (lo que se
...
//      oscuro.
//
// Acá queda centralizado: se guarda la elección, se respeta la del sistema
// cuando no hay elección guardada, y se aplica sobre <html>.
...
// hidratar React saltaría a oscuro: ese parpadeo blanco es lo que se siente
// como "app hecha a las apuradas".
export const THEME_BOOT_SCRIPT = `(function(){try{var k='${THEME_STORAGE_KEY}';var s=localStorage.getItem(k);var t=(s==='light'||s==='dark')?s:(window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light');document.documentElement.dataset.theme=t;var m=document.querySelector('meta[name="theme-color"]');if(m)m.setAttribute('content',t==='dark'?'${THEME_BG.dark}':'${THEME_BG.light}');}catch(e){}})();`;
